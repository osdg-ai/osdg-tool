...
class NgramMatcher:
    def __init__(self,
        ...
        if singularize:
            ngrams = self._singularize(ngrams)
        self.__singularize = singularize
    ...
    def __validate_ngrams(ngrams) -> NoReturn:
        ...
        elif len(ngrams) != len(set(ngrams)):
    ...
    def _singularize(self, ngrams: Union[str, Iterable[str]]):
        if isinstance(ngrams, str):
            singular_tokens = list()
            for token in ngrams.split():
                if token.lower() not in self.stop_words:
                    singular_tokens.append(
                        self.pluralizer.singular(token))
                else:
                    singular_tokens.append(token)
            return ' '.join(singular_tokens)
        else:
            return [self._singularize(ngram) for ngram in ngrams]
    ...
    def _generate_ngrams(self, document: str) -> List[str]:
        ...
        return self._singularize(ngrams) if self.__singularize else ngrams
